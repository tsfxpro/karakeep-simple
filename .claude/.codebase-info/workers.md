...
- Browser sessions (`crawler/browser.ts`, `crawler/browserSession.ts`): connect via `BROWSER_WEBSOCKET_URL` (Playwright protocol), `BROWSER_CDP_URL` (CDP, used as-is), or `BROWSER_WEB_URL` (host resolved to an IP only for `http:`/`ws:`). `BROWSER_CONNECT_ONDEMAND` opens a session per crawl instead of a persistent one. `BROWSER_MAX_CONCURRENT_SESSIONS` caps sessions across both crawler queues, and `BROWSER_SESSION_BUDGET_SEC` (`SessionBudget`) fits navigation, the network-idle wait, autoconsent and screenshot/PDF capture into a hosted browser's session limit. A failed connect falls back to a plain fetch when `BROWSER_FALLBACK_TO_FETCH` is on. Crawler retries come from `CRAWLER_NUM_RETRIES`.
